...
기본은 인메모리다. `Storage:Path`를 주면 저장소가 그 디렉터리에 **추가 전용 저널**(JSON Lines)로
남아 재시작에 살아남는다. 켜지 않으면 지금까지와 똑같이 돌고, 그래서 테스트와 CI는 디스크를
건드리지 않는다.

```bash
Storage__Path=/var/lib/chopilot dotnet run --project src/ChoPilot.Server
curl localhost:5080/v1/storage   # durable 여부 · 복원 건수 · 손상 줄 수
```

저장소마다 **남기는 것이 다르다**: 매핑 캐시·관측·판단은 결과 레코드를, UEP와 엔티티는
**입력**을(상태가 접기 결과라 접는 규칙이 바뀌면 다시 접혀야 한다), 지식은 **연산**을
(승인·폐기가 대기본을 지우는 부수효과를 가져서) 남긴다. 기반 마스터는 **남기지 않는다** —
외부 출처에서 다시 받는 것이 맞고, 관측 파생물을 마스터로 굳히면 계보가 뒤집힌다.

쓰기 도중 종료돼 마지막 줄이 잘리면 그 줄만 건너뛰고 **세어서 보고**한다(콘솔 상단 배지와
`/v1/storage`). 전체를 버리면 멀쩡한 앞부분까지 잃고, 조용히 넘어가면 유실을 아무도 모른다.

설계 근거는 [ARCHITECTURE.md](ARCHITECTURE.md) §12. 운영은 여전히 Aurora/DynamoDB다.

### 자동 관측 — 원클릭 또는 반복

`chopilot-dump`는 한 번 찍고 끝나는 도구였다. 이제 반복 모드가 있다.

```bash
chopilot-watch.cmd                       # 원클릭 — 더블클릭하면 10초 간격으로 돈다
chopilot-dump --watch --upload           # 같은 것을 명령줄로
chopilot-dump --watch 30 --rounds 20 --upload
```

**화면이 그대로면 보내지 않는다.** 판정은 값까지 포함한 지문으로 한다 — 서명(구조만 해싱)과
일부러 다르다. 같은 화면에서 수량만 고친 것은 **같은 서명이지만 다른 사건**이고, 진행률과
완료 증거의 원자료다. 이 건너뜀이 없으면 아무 일 없는 화면이 계속 쌓여 적중률이
학습이 아니라 반복 횟수를 센 값이 된다.

**동의 게이트는 매 회차 다시 본다.** 한 번만 보고 반복하면 사용자가 도중에 제외 대상 앱으로
옮겨가도 계속 캡처된다 — 그건 동의가 아니다.

루프 자체(간격·건너뜀·오류 내성·취소)는 `ChoPilot.Core`의 `ObservationLoop`에 있다.
클라이언트는 `net8.0-windows`라 Linux에서 컴파일되지 않으므로, 판단을 Core에 두어야
UIA 없이 시험할 수 있다 — 클라이언트에 남는 것은 UIA 호출 한 줄뿐이다.

> `--completed`와 `--watch`는 함께 쓸 수 없고 실행 시 거부된다. 완료 신호는 "저장을 눌렀다"는
> 1회성 판단이라, 반복 캡처에 붙이면 작성 중간 화면이 전부 완료로 기록된다.

### 작업 완료 신호 — 필수 필드 규칙을 추측에서 관측으로

시드의 `rule.required.PurchaseRequest`(품목·수량·납기·거래처)는 **아무도 확인한 적 없는
추측**이고, 가이드의 "…입력이 남았습니다" 제안이 전부 그 위에 서 있다.

```bash
chopilot-dump --completed --upload    # 저장 버튼을 누른 직후 캡처 → trigger=save_clicked
curl localhost:5080/v1/completions    # 개념별 채움률
```

작성 **중간**의 화면은 증거가 아니다 — 빈칸이 미완인지 불필요인지 구분되지 않는다.
저장 시점의 화면만 집계되고, 채움률의 분모는 **완료 건수가 아니라 그 개념이 화면에 있던
횟수**다(그 필드가 없는 화면 변형이 규칙을 흔들지 않게).

```
3명이 납기를 빈 채로 저장 → 채움률 0%
→ aggregate → rule.required.PurchaseRequest 개정 초안 (DeliveryDate 삭제)
→ approve(kim) → 지식 v1 → v2
→ 같은 관측의 가이드: 3/4 (75%, "납기 입력이 남았습니다") → 3/3 (100%, 힌트 없음)
```

판정은 세 갈래다: 채움률 ≥90% 필수, ≤50% 비필수, **그 사이는 손대지 않는다**.
애매한 증거로 규칙을 흔들면 가이드가 배치마다 말을 바꾼다. 마스킹된 민감 필드는
**채움으로 센다** — `mask`는 값을 대체할 뿐 지우지 않으므로 사용자는 실제로 입력했다.

> `chopilot-dump`는 스냅숏 도구라 클릭을 관측할 수 없다. `--completed`는 사람이 붙이는
> 표시이고, 상주 에이전트가 저장 클릭을 자동 관측하는 것은 Phase 2다(ARCHITECTURE §11).

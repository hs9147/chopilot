...
| ④ 검수 큐 &amp; 보정 | 저신뢰 매핑을 **보정하거나 승격**해 적중률을 끌어올린다 (H3b 해소) | 수기 판단 |
...
**④ 검수 큐가 적중률의 손잡이다.** 적중률이 0에서 움직이지 않는다면 대개 저신뢰 매핑이 쌓여 있는
것이다(T1). AI 재호출은 백오프가 막지만(T1b) 신뢰도는 스스로 오르지 않는다 — 사람이 여기서 고쳐야 한다.

| 조치 | 쓰는 때 | 결과 |
|------|--------|------|
| **보정 저장 (개인)** | 매핑이 **틀렸다** — 개념을 고쳐야 한다 | `personal:{측정자}`에 신뢰도 1.0. 그 사람에게만 적용, 이후 AI 호출 0 |
| **그대로 승격 (공용)** | 매핑은 **맞는데** 신뢰도만 낮다 | 지금 스코프에서 `trusted`로. 모두에게 적용 |

- 개념은 별칭으로도 받는다(`단가` → `UnitPrice`). **온톨로지에 없는 개념이 하나라도 있으면 전체가 거부된다** —
  민감 여부를 모르는 개념이 신뢰도 1.0으로 굳으면 마스킹 2차 방어선이 무력해지기 때문이다.
- 측정자 ID를 먼저 넣어야 한다. 보정·승격은 `/v1/decisions`에 **누가 언제 무엇을** 승인했는지로 남고,
  그 이력은 리포트에 함께 실린다.
- 보정해도 검수 큐가 줄지 않는 것이 정상이다 — 보정은 개인 평면을 고치고, 큐는 공용 평면을 보여준다.
  큐를 비우는 건 승격이다.
...
### H5 — 결정론 연결 정밀도 ≥ 0.9 (**1단 구현 · 2·3단 수기**)

Entity Resolver의 **1단(Deterministic, 공유 키 매칭)** 은 구현돼 있다. 2단(Temporal)과
3단(Semantic)은 메일·문서 관측이 전제라 여전히 수기다(ARCHITECTURE §6).

**(1) ERP 축 자동 진단** — 재생 후 엔티티 결정 결과를 본다:

```bash
curl -s http://127.0.0.1:5080/v1/entities \
  | jq -c '{count, splitCandidates, entities: [.entities[] | {type, key, mentions, variants}]}'
```

- `splitCandidates`가 **진짜 진단값이다.** 구두점·공백까지 지운 형태가 같은데 키가 갈린 경우
  (`M-001` vs `M001`)를 보고한다. **자동 병합하지 않는다** — 잘못 합치면 서로 다른 품목이
  하나가 되고 그 오류가 이후 모든 품목 지식에 전파된다. 갈린 채로 두는 쪽이 회복 가능하다.
- `variants`는 정규화 전 원문이다. 같은 키 아래 표기가 여러 개면 정규화가 일하고 있다는 뜻.
- `links`는 같은 업무객체 안의 공동 출현 — 품목 축 지식의 원천이다.

**(2) 정밀도 채점** — `splits`에 나온 각 후보가 실제로 같은 실체인지 사람이 판정한다.
정밀도 = 맞게 연결한 건수 / 연결한 건수 전체. **분모는 "연결한 건"이지 "연결해야 했던 건"이
아니다**(정밀도이므로).

**(3) 2·3단(메일·문서)** — 여전히 수기다. KIT §5의 규칙 순위(문서번호 일치 → 거래처코드 →
첨부·시간창)로 손으로 매칭하고, 모호 케이스는 따로 모은다 — Phase 2 설계의 입력이 된다.

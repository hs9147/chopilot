...
> **`distinctSignatures`가 진짜 진단값이다.** 캡처한 화면이 3종인데 이 값이 3보다 크면, 같은 화면이 여러 서명으로 갈라졌다는 뜻이고 적중률은 절대 95%에 못 간다. 원인은 §1.2의 T4(절단)이거나 화면에 남아 있는 동적 구조다. 갈린 두 스냅샷의 구조를 직접 비교하려면 `SignatureService.Skeleton()`이 해시 이전 형태를 돌려준다.

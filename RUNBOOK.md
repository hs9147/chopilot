...
실행 가능성을 유용성과 뭉치면 옳게 찾아낸 종류가 조직 사정 때문에 조용히 꺼진다.
그래서 표에는 보이되 기준은 건드리지 않고, 계속 낮으면 노트로 알린다.

**종류별 문턱** — 정확성 게이트를 먼저 보고, 통과하면 품질 `(정확성+유용성)/2`로 움직인다.

| 조건 | 동작 |
|---|---|
| 평가 5건 미만 | 건드리지 않는다 |
| **정확성 평균 < 2.0** | **그 종류를 끈다** — 문턱을 올리는 우회로를 거치지 않는다 |
| 품질 < 2.5 | 문턱 +0.10 (상한 0.80) |
| 상한에서도 < 2.5 | 그 종류를 끈다 |
| 품질 > 3.8 | 문턱 −0.05 (하한 0.20) |
| 2.5 ~ 3.8 | 흔들지 않는다 |
...
> 위 조합의 품질은 `(1+4)/2 = 2.5`라 **조정 구간 안**이다. 한 숫자로 뭉쳐 있었다면 아무 일도
> 일어나지 않았을 자리인데, 정확성 축이 잡아내 그 종류를 끈다:

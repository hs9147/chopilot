...
**제외**는 그 추정을 캐시에서 지운다. 되돌리기가 아니라 **다시 묻게 하는 것**이다:

- 엔트리가 사라지면 **재추론 백오프의 기준도 함께 사라져** 다음 관측에서 곧바로 AI를 다시 부른다.
  **추론 비용이 다시 난다.** 판단이 틀렸을 때 쓰는 것이지 마음에 안 들 때 누르는 버튼이 아니다.
- 공용 스코프(`global`·`org:*`)를 지우면 **모두에게 영향이 간다**. 그래서 누가 지웠는지가
  아래 `최근 결정`에 `inference_discard`로 남는다.
- **남의 개인 스코프는 지울 수 없다** — 404로 떨어지고 존재 여부도 알려주지 않는다.
...
**제외한 판단도 목록에 남는다.** 캐시에서는 사라지지만 결정 이력에서 끌어와 회색 행으로
같은 시간축에 세운다. 안 그러면 "아직 추론된 적 없음"과 "방금 내가 제외함"이 화면에서
똑같이 보인다. 제외한 화면이 다시 관측되면 새 추론 행이 그 위에 얹혀, 경과가 위에서
아래로 읽힌다:

```
18:40  B화면  PurchaseRequest  1  0.60 θ 미만   검수 대기 global   [수정][제외]
18:39  B화면  필드 1개 제외 — 다음 관측에서 재추론된다   제외됨 global · hong
```

...
**자체 평가.** 생성 버튼은 먼저 사람의 결정을 되읽어 기준을 고친다:

- 종류별 결정이 **5건 미만이면 건드리지 않는다.** 두세 번 기각됐다고 종류를 끄면 그 종류는
  다시 제안되지 않으므로 **기각이 맞았는지 확인할 표본이 영원히 늘지 않는다.**
- 채택률 25% 미만 → 문턱 +0.10 (상한 0.80)
- 상한에서도 25% 미만 → **그 종류를 끈다** (점수가 아니라 종류가 틀린 것이다). 이유를 함께 적는다.
- 채택률 70% 초과 → 문턱 −0.05 (하한 0.20) — 더 올린다
- 그 사이(25~70%)는 흔들지 않는다. 매번 움직이면 기준이 잡음을 따라간다.
...
curl -s -X POST -H 'X-ChoPilot-User: hong' -H 'Content-Type: application/json' \
  -d '{"accept":false,"note":"근거는 맞지만 이번 분기엔 손댈 수 없다"}' \
  localhost:5080/v1/proposals/rework%3Asig-a~sig-b/decide
```

- **결정된 제안은 다시 올라오지 않는다.** 안 그러면 기각한 제안이 매 생성마다 되살아난다.
- 두 번째 결정은 404다 — 결정이 뒤집히면 채택률이 흔들린다.
- 결정은 `proposal_accepted` / `proposal_rejected`로 결정 이력에 남고, **이것이 자체 평가의
  유일한 입력**이다. 그래서 기각할 때는 사유를 적는 편이 낫다.
- 겹쳐 누르면 **409**다(§4.4와 같은 게이트) — 같은 제안이 두 번 만들어지지 않게.
